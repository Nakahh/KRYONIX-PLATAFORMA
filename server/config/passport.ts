...
const jwtOptions = {
  ...
  secretOrKey:
    process.env.JWT_SECRET || "kryonix-jwt-secret-change-in-production",
  ...
  new JwtStrategy(jwtOptions, async (jwtPayload, done) => {
    try {
      ...
      if (!dataSource) {
        ...
        return done(null, {
          id: jwtPayload.sub,
          email: jwtPayload.email || "dev@kryonix.com.br",
          isActive: true,
        });
      }
    ...
    async (req, email, password, done) => {
      try {
        ...
        if (!dataSource) {
          ...
          return done(null, false, { message: "Sistema em manutenção" });
        }
        ...
        if (!user) {
          return done(null, false, {
            message:
              "Email não encontrado. Verifique seus dados ou crie uma conta.",
          });
        }
        ...
        if (user.twoFactorEnabled && req.body.twoFactorCode) {
          ...
          if (!isValid2FA) {
            return done(null, false, {
              message:
                "Código de autenticação inválido. Verifique no seu app autenticador.",
            });
          }
        }
...
passport.deserializeUser(async (sessionUser: any, done) => {
  try {
    const dataSource = getDataSource();
    if (!dataSource) {
      // Modo sem banco - retornar usuário fake para desenvolvimento
      return done(null, {
        id: sessionUser.id,
        email: "dev@kryonix.com.br",
        name: "Dev User",
        role: "admin",
        tenantId: sessionUser.tenantId || "dev-tenant",
        permissions: ["admin"],
        isActive: true,
      });
    }
