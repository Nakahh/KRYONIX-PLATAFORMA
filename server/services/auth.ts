...
export class AuthService {
   ...
   */
  async oauthLogin(provider: string, profile: any): Promise<{
    user: UserSession;
    tokens: AuthTokens;
    isNewUser: boolean;
  } | null> {
  ...
  async validateTwoFactorCode(userId: string, code: string): Promise<boolean> {
    try {
      const validation = await twoFactorService.validateTwoFactorCode(userId, code);
  ...
  private generateSubdomain(email: string): string {
    const baseSubdomain = email.split('@')[0]
      .toLowerCase()
      .replace(/[^a-z0-9]/g, '')
      .substring(0, 10);
