...
# ============================================================================
# 🚀 INSTALADOR KRYONIX PLATFORM - VERSÃO CORRIGIDA FUNCIONAL
# ============================================================================
# Autor: Vitor Fernandes
# Descrição: Instalador 100% automático baseado no que funcionava
# CORREÇÃO: webhook-deploy.sh criado ANTES do Docker build
# ============================================================================
...
# CORREÇÃO CRÍTICA: Criar webhook-deploy.sh ANTES do Docker build
log_info "🔗 Criando webhook-deploy.sh (OBRIGATÓRIO para Docker build)..."
...
deploy() {
    ...
    # CORREÇÃO CRÍTICA: Recriar webhook-deploy.sh após o clone
    if [ ! -f "webhook-deploy.sh" ]; then
        log "🔧 Recriando webhook-deploy.sh após clone..."
        cat > webhook-deploy.sh << 'INNER_WEBHOOK_EOF'
#!/bin/bash
echo "🚀 KRYONIX Deploy executado em $(date)"
...
log_success "✅ webhook-deploy.sh criado (CRÍTICO para Docker build)"
...
required_files=("package.json" "server.js" "webhook-listener.js" "kryonix-monitor.js" "webhook-deploy.sh" "public/index.html")
missing_files=()
...
echo -e "    ${BLUE}│${RESET} ✅ webhook-deploy.sh criado ANTES do Docker build"
